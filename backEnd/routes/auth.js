...
router.post(
  ...
  async (req, res) => {
    ...
    if(user){
      return res.status(400).json({success, errors:"User with email already exist" });
    }
...
router.post(
  "/login",
  [ body("email","please enter a valid email").isEmail(),body("password","please enter a valid password").exists() ],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      const success= false;
